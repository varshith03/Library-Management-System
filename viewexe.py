...
class ViewBook():
# View All Books Function for admin
    def viewbookfun(self):
        ...
        cursor=con.cursor()
        v='yes'
        s="Select * from book where Visible='%s'"
        arg=(v)
    ...
    # View All Books Function for member
    def memviewbookfun(self):
        self.tableWidget.clear()
        ...
        cursor=con.cursor()
        v='yes'
        s="Select BookID,Name,Author,Dept,Avail from book where Visible='%s'"
        arg=(v)
        cursor.execute(s%arg)
        rows=cursor.fetchall()
        for row in rows:
